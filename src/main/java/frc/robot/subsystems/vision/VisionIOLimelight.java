...
public class VisionIOLimelight implements VisionIO {
  ...
  public void update() {
    Optional<PoseEstimate> leftEstimate =
        pollLL(LeftLimelightConstants.CAMERA_NAME, previousLeftEstimate);
    Optional<PoseEstimate> rightEstimate =
        pollLL(RightLimelightConstants.CAMERA_NAME, previousRightEstimate);
    ...
    if (MathHelpers.chassisSpeedsNorm(robotState.getChassisSpeeds()) < 3.0
      ...
      if (leftEstimate.isPresent() && leftEstimate.get().rawFiducials.length > 0) {
        double closestTagDist =
            Arrays.stream(leftEstimate.get().rawFiducials)
                .mapToDouble(fiducial -> fiducial.distToCamera)
                .min()
                .getAsDouble();
        leftStdDev =
            xyStdDevCoefficient * Math.pow(closestTagDist, 2) / leftEstimate.get().tagCount;
        leftHeadingStdDev =
            thetaStdDevCoefficient * Math.pow(closestTagDist, 2) / leftEstimate.get().tagCount;
      ...
      if (rightEstimate.isPresent() && rightEstimate.get().rawFiducials.length > 0) {
        double closestTagDist =
            Arrays.stream(rightEstimate.get().rawFiducials)
                .mapToDouble(fiducial -> fiducial.distToCamera)
                .min()
                .getAsDouble();
        rightStdDev =
            xyStdDevCoefficient * Math.pow(closestTagDist, 2) / rightEstimate.get().tagCount;
        rightHeadingStdDev =
      ...
      if (leftStdDev < rightStdDev) {
        ...
        robotState.seenReefFaceID(
            (int) LimelightHelpers.getFiducialID(RightLimelightConstants.CAMERA_NAME));
      }
    }
  }
