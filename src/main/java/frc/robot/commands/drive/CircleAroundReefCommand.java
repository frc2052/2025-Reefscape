...
public class CircleAroundReefCommand extends Command {
  ...
  public CircleAroundReefCommand(
      double radius, double Speed, boolean isMovingRight, DrivetrainSubsystem drivetrain) {
    this.radius = radius;
    this.Speed = Speed;
  ...
  public void execute() {
    ...
    double heading =
        Math.atan2(
            robotState.getFieldToRobot().getY() - signChanger(yVelocity),
            robotState.getFieldToRobot().getX() - xVelocity);
    ...
    new DefaultDriveCommand(
        xVelocitySupplier, yVelocitySupplier, rotationVelocitySupplier, () -> false);
  }
