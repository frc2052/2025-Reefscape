        ...
        FieldElementFace face;
         face = AlignmentCommandFactory.idToReefFace(
            ...
            if (face == FieldElementFace.AB || face == FieldElementFace.EF || face == FieldElementFace.IJ) {
                return Commands.runOnce(()->SuperstructureSubsystem.getInstance().setCurrentAction(TargetAction.UA),SuperstructureSubsystem.getInstance());
            } else {
                return Commands.runOnce(()->SuperstructureSubsystem.getInstance().setCurrentAction(TargetAction.LA),SuperstructureSubsystem.getInstance());
                        }
