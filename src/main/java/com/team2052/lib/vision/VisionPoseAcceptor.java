...
public class VisionPoseAcceptor {
    ...
    public boolean shouldAccept(
        ...
        if (estimatedPose.getTranslation().getX() < -VisionConstants.FIELD_BORDER_MARGIN.in(Meters)
                || estimatedPose.getTranslation().getX() > FieldConstants.FIELD_LENGTH.in(Meters)
                ...
                || estimatedPose.getTranslation().getY() > FieldConstants.FIELD_WIDTH.in(Meters)
                        + VisionConstants.FIELD_BORDER_MARGIN.in(Meters)) {
            return false;
        }
        ...
        if (isInAuto) {
            if (estimatedPose.getTranslation().toTranslation2d().getDistance(robotPose.getTranslation()) > VisionConstants.MAX_VISION_CORRECTION.in(Meters)) {
                return false;
            }
        }
